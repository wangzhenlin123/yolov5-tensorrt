...
public:
    ...
    {
        return mData.data() + (mBatchCount * mBatchSize * TensorRTCommon::volume(mDims));
    }
...
private:
    void readDataFile(const std::string& dataFilePath)
    {
        ...
        magicNumber = TensorRTCommon::swapEndianness(magicNumber);
        ...
        numImages = TensorRTCommon::swapEndianness(numImages);
        imageH = TensorRTCommon::swapEndianness(imageH);
        imageW = TensorRTCommon::swapEndianness(imageW);
...
private:
    ...
    bool update()
    {
        ...
        else
        {
            ...
            Logging::gLogInfo << "Batch #" << mFileCount << std::endl;
            ...
            for (int i = 1; i <= mBatchSize; i++)
            {
                ...
                sName = sName + ".ppm";
                Logging::gLogInfo << "Calibrating with file " << sName << std::endl;
            ...
            std::vector<TensorRTCommon::PPM<imageC, imageH, imageW>> ppms(fNames.size());
            for (uint32_t i = 0; i < fNames.size(); ++i)
            {
                readPPMFile(locateFile(fNames[i], mDataDir), ppms[i]);
            }

            std::vector<float> data(TensorRTCommon::volume(mDims));
